...
leagues = {'epl':'premier-league', 'liga':'la-liga',
            'bundes':'bundesliga', 'serie':'serie-a', 
            'un':'ligue-1'} #, 'mls':'mls',}

for k in leagues.keys():
    
    l = leagues[k]
    print("Extraction data for {} ...".format(l))

    csv_file = '{}_2017_predictions.csv'.format(k)

    time.sleep(10)
    r = urllib.request.urlopen('{}{}/'.format(url,l)).read()
